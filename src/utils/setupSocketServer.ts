...
const setupSockerServer = (server: http.Server) => {
  ...
  const disconnect = (client: Socket) => {
    ...
    if (user) {
      ...
      console.log(
        `Socket Id - ${client.id!} : User with Id - ${user.id} has disconnected from socket server`,
      );
  ...
  io.on('connection', async (client: Socket) => {
    ...
    if (user) {
      console.log(
        ...
        client.handshake.auth.user.name,
      );
      ...
      if (!alreadyConnected) {
        ...
        console.log(
          `Socket Id - ${client.id} assigned to User with Id - ${user.id}`,
        );
      } else {
        console.log(
          `Duplicate entry for userId - ${user.id} & username - ${user.name}`,
        );
        client.disconnect();
      }
    } else {
